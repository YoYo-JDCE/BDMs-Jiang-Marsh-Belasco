...
function Z1Individual20yearandGao
%% FORMULATION

%format short

% Model parameters
a = [30360 1 0.1 0.005 500 0];
b = [0 0 1197.9 -0.5];
nu =[28785.01885 10];
...
digits(20)
% Steady-state
qstar =min(1,((nu(2)+w*(-b(2)+b(4)*z)+a(2)*p)*(1-delta*ita(3))*(1-ita(3))+(a(6)...
...
lstar1 = p*(-a(3)-a(5)*sstar1-a(6)*qstar1)/(-delta*ita(3)+1);                            % Shadow price for pollutant without boundary

check = -w*(b(3)+b(4)*qstar1)+delta*(-ita(4)-ita(5)*qstar1)*lstar1;
fprintf('check the condition that the optimal z=0. Check value <0 %5.10f\n' , check)
% Check the condition that the FOC=0
check1=-a(2)*p-a(4)*qstar1*p-a(6)*sstar1*p-nu(2)+w*b(2)-b(4)*z*w+delta*lstar1*(-ita(2)-ita(5)*z);
fprintf('Check the condition that the FOC=0 %5.10f\n' , check1)
